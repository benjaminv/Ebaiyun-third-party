...
//进行中的拼团
function user_having_extpintuan_count()
{
	$sql = 'SELECT COUNT(*) ' . 'FROM ' . $GLOBALS['ecs']->table('extpintuan_orders') . 'WHERE follow_user  = \'' . $_SESSION['user_id'] . '\' and status not in(1,2,3,4)';
	return $GLOBALS['db']->getOne($sql);
}
