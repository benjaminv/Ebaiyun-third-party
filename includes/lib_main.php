...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email,u.user_rank, u.user_name, u.user_money, u.pay_points,u.headimg'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    if ($filename != 'index')
    {
        ...
        {
            /* 团购 */
            if ('extpintuan' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['group_buy_goods'] . '_' . $page_title;
                $args       = array('gbid' => '0');
                //$ur_here   .= ' <span class="crumbs-arrow">&gt;</span> <a href="extpintuan.php?act=userlist" class="last">'.'我的团购</a>';
                $ur_here .= '<span class="crumbs-arrow">&gt;</span><span class="last">用户中心</span>';
            }
            /* 拍卖 */
