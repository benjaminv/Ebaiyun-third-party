...
//佰云币兑换积分
function action_change_point() {
	$point = $_GET['point'];
	if ($point*1 <= 0 || empty($_SESSION['user_id'])) {
		show_message("请输入有效的积分数量！", '', '', 'error');
	}
	$ecs = $GLOBALS['ecs'];

	//判断当前账户余额是否可以兑换积分
	$cou_sql = 'SELECT COUNT(*) ' . 'FROM ' . $ecs->table('users') . " where user_id='".$_SESSION['user_id']."' and (user_money-$point)>=0";
	$cou_num = $GLOBALS['db']->getOne($cou_sql);
	if (!$cou_num) {
		show_message('账户余额不足', array('返回上一页','点击去充值'), array('user.php', 'user.php?act=account_deposit'), 'info');
	}
	//更新账户余额跟积分数量
	$sql = "UPDATE " . $ecs->table('users') . " SET pay_points = pay_points+".$point.", user_money = user_money-".$point." WHERE  user_id='".$_SESSION['user_id']."' and user_money>0";
	$res = $GLOBALS['db']->query($sql);
    show_message('积分兑换成功', '点击返回', 'user.php', 'info');
}
...
function action_default ()
{
	...
	$ecs = $GLOBALS['ecs'];
	$user_id = $_SESSION['user_id'];
	/* 代码注释 by  _star 路由前已调用会员信息，去掉重复调用*/
	$rank = get_rank_info();
	/* 	include_once (ROOT_PATH . 'includes/lib_clips.php');
	 if($rank = get_rank_info())
	 {
	...
	$smarty->assign('guanzhu', get_follow_shops($user_id)); // 获取用户关注的店铺
...
//    $smarty->assign('user_notice', $_CFG['user_notice']);
    ...
    /* 代码修改 By   End */
    $pin_sql = 'SELECT COUNT(*) ' . 'FROM ' . $GLOBALS['ecs']->table('extpintuan_orders') . ' AS pto  ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('extpintuan') . ' AS pt ON pto.pt_id   = pt.pt_id   ' . 'WHERE follow_user  = \'' . $_SESSION['user_id'] . '\' and pt.status not in(1,2,3,4) ';
	$pintuan_num = $GLOBALS['db']->getOne($pin_sql);
    $smarty->assign('pintuan_num', $pintuan_num); //当前用户拼团(进行中)数量

    $draw_sql = 'SELECT COUNT(DISTINCT lbd.order_id) ' . 'FROM ' . $GLOBALS['ecs']->table('lucky_buy_detail') . 'AS lbd LEFT JOIN ' . $GLOBALS['ecs']->table('lucky_buy') . ' AS lb ON lbd.lucky_buy_id   = lb.lucky_buy_id   '. 'WHERE user_id  = \'' . $_SESSION['user_id'] . '\' and lb.status != 1 ';
	$draw_num = $GLOBALS['db']->getOne($draw_sql);

	$smarty->assign('draw_num', $draw_num); //当前用户开奖(进行中)数量
	$ex_where = " and user_id=$user_id";
    $dai_num = $db->GetOne('SELECT COUNT(*)' . ' FROM ' . $ecs->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('await_ship')) ;
    $smarty->assign('dai_num', $dai_num); //当前待发货数量

	$dai_fu_num = $db->GetOne('SELECT COUNT(*)' . ' FROM ' . $ecs->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('await_pay')) ;    
	$smarty->assign('dai_fu_num', $dai_fu_num); //待付款数量
...
function action_order_list ()
{
	...
	/* 待发货的订单： */
	//获取所有已申请退款的订单ID
	 $tui_sql = "SELECT order_id FROM " . $GLOBALS['ecs']->table('back_order'). " where status_back<6";
    $tui_id = $GLOBALS['db']->getCol($tui_sql);
    
    //拼团失败的
    $tuan_false_sql = 'SELECT DISTINCT lbd.order_id ' . 'FROM ' . $GLOBALS['ecs']->table('lucky_buy_detail') . 'AS lbd LEFT JOIN ' . $GLOBALS['ecs']->table('lucky_buy') . ' AS lb ON lbd.lucky_buy_id   = lb.lucky_buy_id   '. 'WHERE user_id  = \'' . $user_id . '\' and lb.status = 2';
	$tuan_false_id = $GLOBALS['db']->getCol($tuan_false_sql);
	$tui_id = array_merge($tui_id, $tuan_false_id);
	$tui_where = "";
    if (!empty($tui_id)) {
    	$tui_where = " and order_id not in(".implode(',', $tui_id).")";
    }
	$order_count['await_ship'] = $db->GetOne('SELECT COUNT(*)' . ' FROM ' . $ecs->table('order_info') . " WHERE 1 $ex_where ". $tui_where. order_query_sql('await_ship'));
	...
	switch($composite_status)
	{
		...
		case CS_AWAIT_SHIP:
			//获取所有已申请退款的订单ID
			if (!empty($tui_where)){$where = $tui_where;}
	...
	if($have_tuikuan)
	{
		if ($_POST['order_all'])
		{
			...
			if(empty($price_refund)){
				$price_refund1 = $GLOBALS['db']->getOne("SELECT goods_amount FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = " . $order_id);
				$price_refund2 = $GLOBALS['db']->getOne("SELECT shipping_fee FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = " . $order_id);
				$price_refund =$price_refund1 + $price_refund2;
			}
		}
		else
		{
...
function action_add_book_goods ()
{
	...
	if(! preg_match("/^1(3|5|8)[0-9]{1}[0-9]{8}$|15[0189]{1}[0-9]{8}$|17[013678]{8}$|189[0-9]{8}$/", $tel))
	{
		$result['error'] = 3;
