...
include_once(MROOT_PATH . 'weixin/weixin_oauth.php');  //授权登陆文件 如果贵站没有这项功能请屏蔽
require_once(MROOT_PATH . 'includes/prince/lib_extpintuan.php');
include_once(MROOT_PATH . 'wxm_extpintuan.php');  
...
$smarty->assign('lang', $_LANG);       // 网店帮助
$smarty->assign('is_extpintuan', 'true');
    ...
    if (is_array($goodslist)) {
        foreach ($goodslist as $vo) {
           ...
           $vo['sold'] =$vo['virtual_sold']+selled_count($vo['goods_id']) ;
				...
				$sayList[] = array(
					'pro-inner' => '
						...
						<div class="proInfo-wrap"> <a href="' . $vo['url'] . '" >
						  ...
						  <br /><div  class="mkPrice"  style="display:none">
							<em>市场价：</em> 
			...
			<a href="'.$vo['url'].'"  > <img src="../' . $img_url . '" alt="' . $vo['goods_name'] . '"> </a>
...
if ($_REQUEST['act'] == 'asyncuserlist')
{
    ...
    if (is_array($goodslist)) {
        foreach ($goodslist as $vo) {
            ...
            $sayList[] = array(
                'pro-inner' => '<div>
        <div class="proImg-wrap"> <a href="' . $vo['url'] . '" > <img style="height:100px; width:auto;" src="../' . $img_url . '" alt="' . $vo['goods_name'] . '" style="margin-left:5px;"   > </a> 		
		</div>
        <table style="position:absolute; right:0px;top:10px;"><tr><td><div class="ptInfo-wrap"> <a href="' . $vo['url'] . '" >
          <div class="proTitle" style="font-size:12px;line-height:16px; text-align:right; padding-right:20px;height:auto; " >' . $vo['act_name'] . '</div>
          <div class="ptPrice">
...
{   
    ...
    if (!$smarty->is_cached('extpintuan_goods.dwt', $cache_id))
    {
       ...
       // $extpintuan['need_follow'].'-'.$weixininfo['isfollow'].'-'.is_wechat_browser_for_extpintuan(); 
		...
		require_once MROOT_PATH."wxjs/jssdk.php";
...
{    	
    ...
    /* 更新：清空购物车中所有拼团商品 */
    include_once(MROOT_PATH . 'includes/lib_order.php');
