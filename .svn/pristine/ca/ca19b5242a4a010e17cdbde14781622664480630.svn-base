<?php
function alipay_pay($order){
	$parameter = array(
		"service" => "mobile.securitypay.pay",
		"partner" => "2088521415688244",
		"seller_id" => "2088521415688244",
		"payment_type"	=> 1,
		"notify_url"	=> $GLOBALS['ecs']->url().'pay/huidiao.php',
	...
	exit(json_encode(array('code'=>200,'result'=>$str)));
}
	  
function weixin_pay($order){
	...
	$package['body'] = $order ['order_sn'];
	$package['out_trade_no'] = $order ['order_id'];
	$package['total_fee'] = $order['order_amount']*100;
	$package['spbill_create_ip'] = $_SERVER['REMOTE_ADDR'];
	$package['notify_url'] = NOTIFY_URL; 
	$package['product_id'] = $order['order_sn'];
	...
	if (! empty($data)) {
		...
		exit(json_encode(array('result'=>$jsApiParameters)));	
	}else{
		return false;		
	}
}
