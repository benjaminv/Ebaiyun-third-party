...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    $smarty->assign('draw_goods', index_get_draw());      // 抽奖商品
...
function index_get_group_buy()
{
    ...
    if ($limit > 0)
    {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $row['url'] = build_uri('extpintuan', array('gbid' => $row['group_buy_id']));
...
* 获取抽奖商品
*/
function index_get_draw($page=1) {
    $size = 4;
    $lucky_buy_list = array();
    $lucky_buy_list['finished'] = $lucky_buy_list['finished'] = array();
    $now = gmtime();
    $sql = 'SELECT a.*,g.*, IFNULL(g.goods_thumb, \'\') AS goods_thumb ' . 'FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS a ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON a.goods_id = g.goods_id ' . 'WHERE a.act_type = \'' . GAT_LUCKY_BUY . '\' ' . 'AND a.start_time <= \'' . $now . '\' AND a.end_time >= \'' . $now . '\' AND a.is_finished < 1 ORDER BY rand() limit 4';
